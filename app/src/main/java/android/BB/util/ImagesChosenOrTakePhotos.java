...
/**
 * Created by KalinaRain on 2015/8/30.   封装失败————暂时停止
 * 工具类   https://github.com/hanks-zyh/SelectPicture
 * 拍照后选择图片，或者从图库中选择
 * what you need to to:
 * 通过构造方法实例化，并传入最多可选择的图片数
 * 在onActivityResult方法里面对图片进行相应的处理
 */
public class ImagesChosenOrTakePhotos extends AppCompatActivity{

    /**
     * 最多选择图片的个数
     * 根据实际需要来确定
     */
    private static int max_num;//由于是上传头像，因此只能选一张
     ...
     * @param max_num 最多选择图片的个数
     */
    public ImagesChosenOrTakePhotos(Context context, int max_num) {
    ...
    public void initConfig() throws IllegalParameter {
        if (max_num == 1) {
            //设置右上角的checkbox为不可见，切直接跳转

        } else {
            if (max_num <= 0 || max_num > 9) {
                //抛出异常
                throw new IllegalParameter("请传入正确的个数");
            }
        }
    }
    ...
    private void initView() {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                Log.d("zyh", position + "-------" + currentImageFolder.getDirName() + "----"
    ...
    class PictureAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (position == 0) {
                ...
                holder.checkBox.setVisibility(View.INVISIBLE);//右上角图片不可见
            } else {
                position = position - 1;
    ...
    private void getThumbnail() {
        ...
        for (int i = 0; i < mDirPaths.size(); i++) {
            ...
            Log.d("zyh", i + "-----" + f.getDirName() + "---" + f.images.size());
        }
        tmpDir = null;
    }
    ...
    class IllegalParameter extends Exception {
        public IllegalParameter(String msg) {
            super(msg);
        }
    }
}
