...
import android.BB.ui.linkman.AddContacts;
...
public class MyMessageReceiver extends BroadcastReceiver {
                            ...
                            if(currentUser!=null){//有登陆用户
                                if(toId.equals(currentUser.getObjectId())){
                                    if (ehList.size() > 0) {// 有监听的时候，传递下去
                                        ...
                                        showOtherNotify(context, message.getFromname(), toId,  message.getFromname()+"请求添加好友", AddContacts.class);
                                    }
                                }
                            }
    ...
    public void showOtherNotify(Context context,String username,String toId,String ticker,Class<?> cls){
        ...
        if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
            //同时提醒通知
            BmobNotifyManager.getInstance(context).showNotify(isAllowVoice,isAllowVibrate,R.mipmap.ic_launcher, ticker,username, ticker.toString(),AddContacts.class);
        }
    }
}
